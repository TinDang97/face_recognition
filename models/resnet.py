...
from dpsutil.log import logger
...
class Bottleneck(Features):
    ...
    def forward(self, x):
        ...
        if out.size() != residual.size():
            logger.info_log.info(out.size(), residual.size())
        out += residual
...
class BottleneckNOP(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BottleneckNOP, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        layers = [block(self.inplanes, planes, stride, downsample, dilation=dd)]
...
class ResAdjust(nn.Module):
    ...
    @staticmethod
    def _make_layer(block, plances, dilation, out, number=1):
